"""
RAPIRO-LSA EC2 Backend v2.2 — Bugs corregidos
--------------------------------------------------
Fixes:
  - texto_formado se resetea DESPUES de enviar la respuesta
  - Variable temporal texto_a_hablar para Polly
  - Respuesta incluye texto_hablado para que el Pi sepa que se dijo
...
app = FastAPI(title="RAPIRO-LSA EC2 Backend", version="2.2.0")
...
async def startup_event():
    ...
    logger.info("Inicializando backend v2.2...")
    try:
    ...
    logger.info("Backend v2.2 listo.")
...
@app.get("/health")
def health():
    return {"status": "ok", "service": SERVICE_NAME, "version": "2.2",
...
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  .video-container{position:relative;border-radius:16px;overflow:hidden;border:2px solid #27272a;max-width:700px;width:100%}
  ...
  .cloud-badge{background:#1e1b4b;color:#a855f7;padding:4px 12px;border-radius:20px;font-size:0.7rem;font-weight:600;letter-spacing:1px;margin-top:16px}
...
<body>
  ...
  <div class="info-bar">
    <div class="info-card"><div class="info-label">Letra detectada</div><div class="info-value letra" id="letraVivo">---</div></div>
    <div class="info-card"><div class="info-label">Confianza</div><div class="info-value conf" id="confVivo">0%</div></div>
    <div class="info-card"><div class="info-label">Manos</div><div class="info-value" id="manosVivo">0</div></div>
    <div class="info-card"><div class="info-label">Texto acumulado</div><div class="info-value texto" id="textoVivo">|</div></div>
  </div>
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            progreso = 0.0
            audio_bytes = None
            texto_a_hablar = ""  # FIX: variable temporal para el audio
                    ...
                    if prediccion == letra_candidata:
                        ...
                        if progreso >= 1.0 and not letra_confirmada:
                            ...
                            if prediccion == "FINALIZAR" and texto_formado:
                                # FIX: guardar texto ANTES de resetear
                                texto_a_hablar = texto_formado
                                logger.info(f"FINALIZAR: \"{texto_a_hablar}\"")

                                # Notificar web y DynamoDB con el texto original
                                asyncio.create_task(
                                    enviar_a_web("finalizar", {"texto": texto_a_hablar})
                                )
                                asyncio.create_task(
                                    guardar_evento({
                                        ...
                                        "TextoCompleto": texto_a_hablar,
                                        "Confidence": round(confianza, 4),
                                        "Source": "EC2 WebSocket",
                                    })
                                )

                                # Generar audio con el texto original
                                audio_bytes = await generar_audio(texto_a_hablar)

                                # FIX: resetear DESPUES de generar audio y guardar texto
                                texto_formado = ""
                                letra_candidata = None
                                tiempo_inicio = 0.0
                                letra_confirmada = False
            ...
            # Dibujar frame con landmarks
            # FIX: mostrar texto_a_hablar en el frame si se acaba de finalizar
            ...
            frame_dibujado = dibujar_frame(frame, hand_result, face_result,
                                          prediccion, confianza, texto_mostrar, progreso)
            ...
            # DynamoDB cada 3s si hay confirmacion
            ...
            # FIX: respuesta incluye texto_hablado para que el Pi sepa que se dijo
            respuesta = {
                "letra": prediccion,
                ...
                "manos": num_manos,
                "texto_hablado": texto_a_hablar,
            }

            # Enviar respuesta JSON primero, audio despues
            await websocket.send_json(respuesta)
            if audio_bytes:
                await websocket.send_bytes(audio_bytes)
                logger.info(f"Audio enviado al Pi: {len(audio_bytes)} bytes")
